...
import { laboQuiz, type LaboQuizLang } from '@/lib/translations/labo-quiz'
import { CheckCircle, XCircle } from 'lucide-react'
import { cn } from '@/lib/utils'

export function LaboQuizPanel() {
  const { lang, t } = useLang()
  const [answers, setAnswers] = useState<Record<number, number>>({})
  const [submitted, setSubmitted] = useState(false)

  const quizLang: LaboQuizLang = lang === 'kr' ? 'kr' : lang === 'en' ? 'en' : lang === 'es' ? 'es' : 'fr'
  const questions = laboQuiz[quizLang] ?? laboQuiz.fr
