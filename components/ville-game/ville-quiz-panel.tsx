...
import { villeQuiz, type VilleQuizLang } from '@/lib/translations/ville-quiz'
import { CheckCircle, XCircle } from 'lucide-react'
import { cn } from '@/lib/utils'

export function VilleQuizPanel() {
  const { lang, t } = useLang()
  const [answers, setAnswers] = useState<Record<number, number>>({})
  const [submitted, setSubmitted] = useState(false)

  const quizLang: VilleQuizLang = lang === 'kr' ? 'kr' : lang === 'en' ? 'en' : lang === 'es' ? 'es' : 'fr'
  const questions = villeQuiz[quizLang] ?? villeQuiz.fr
