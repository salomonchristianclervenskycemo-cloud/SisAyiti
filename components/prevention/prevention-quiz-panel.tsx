...
import { preventionQuizzes, type PrevQuizLang } from '@/lib/translations/prevention-quiz'
import { CheckCircle, XCircle } from 'lucide-react'
import { cn } from '@/lib/utils'

type TabIndex = 0 | 1 | 2

export function PreventionQuizPanel({ tabIndex }: { tabIndex: TabIndex }) {
  const { lang, t } = useLang()
  const [answers, setAnswers] = useState<Record<number, number>>({})
  const [submitted, setSubmitted] = useState(false)

  const quizLang: PrevQuizLang = lang === 'kr' ? 'kr' : lang === 'en' ? 'en' : lang === 'es' ? 'es' : 'fr'
  const questions = preventionQuizzes[tabIndex]?.[quizLang] ?? preventionQuizzes[tabIndex]?.fr ?? []
  if (questions.length === 0) return null

  const score = questions.filter((q, i) => answers[i] === q.correct).length

  return (
    <div className="mt-6 p-5 rounded-xl bg-secondary/30 border border-border/50 space-y-4">
              ...
              return (
                <button
                  ...
                  className={cn(
                    ...
                    !isCorrect && !isWrong && isSelected && 'bg-primary/20 border-primary/40',
                    !isCorrect && !isWrong && !isSelected && 'bg-card border-border/50 hover:border-primary/30'
                  )}
                >
      ...
      {!submitted ? (
        ...
        <div className="flex items-center justify-between bg-card p-3 rounded-xl border border-border/50">
          <span className={cn('text-sm font-bold', score === questions.length ? 'text-green-600' : 'text-orange-600')}>
