...
import { diagnosticQuiz, type DiagQuizLang } from '@/lib/translations/diagnostic-quiz'
import { CheckCircle, XCircle } from 'lucide-react'
import { cn } from '@/lib/utils'

export function DiagnosticQuizPanel() {
  const { lang, t } = useLang()
  const [answers, setAnswers] = useState<Record<number, number>>({})
  const [submitted, setSubmitted] = useState(false)

  const quizLang: DiagQuizLang = lang === 'kr' ? 'kr' : lang === 'en' ? 'en' : lang === 'es' ? 'es' : 'fr'
  const questions = diagnosticQuiz[quizLang] ?? diagnosticQuiz.fr
