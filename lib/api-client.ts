...
import {
  ...
  type SeismicStatsResponse,
  type SurveillanceDashboardResponse,
...
export function createApiClient(options: ApiClientOptions = {}) {
  ...
  return {
    ...
    getSeismicEvents(params: { days?: number; minMagnitude?: number; limit?: number; global?: boolean }) {
      ...
      if (params.global === true) q.set("global", "true")
      else if (params.global === false) q.set("global", "false")
      return withBase(`${API_ROUTES.seismicEvents}?${q}`) as Promise<SeismicEventsResponse>
    },

    getSurveillanceDashboard(params: {
      days?: number
      minMagnitude?: number
      global?: boolean
    }) {
      ...
      if (params.days != null) q.set("days", String(params.days))
      if (params.minMagnitude != null) q.set("minMagnitude", String(params.minMagnitude))
      if (params.global === false) q.set("global", "false")
      else q.set("global", "true")
      return withBase(`${API_ROUTES.seismicSurveillance}?${q}`) as Promise<SurveillanceDashboardResponse>
    },
